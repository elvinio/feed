...
feel responsive: section 1 is stored while the rest are still baking, a failed
chunk costs one retry rather than the whole article, and a cancelled run leaves
what finished playable (the dock then offers to finish the rest). Chunks record
...
remembered per article, and Media Session wires up the lock-screen controls.
